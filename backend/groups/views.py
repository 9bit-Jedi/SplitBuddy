...
from .serializers import (AddMemberSerializer, GroupDetailSerializer,
...
class GroupViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return self.request.user.member_groups.prefetch_related(
            "members", "expenses__splits"
        ).distinct()
...
class GroupMembersView(APIView):
    ...
    def post(self, request, pk):
        ...
        if group.members.filter(id=user.id).exists():
            return Response(
                {"detail": "User already a member"}, status=status.HTTP_400_BAD_REQUEST
            )
        if user == request.user:
            return Response(
                {"detail": "Cannot add yourself"}, status=status.HTTP_400_BAD_REQUEST
            )
        group.members.add(user)
        group_serializer = GroupDetailSerializer(group)
        response = {"detail": "Member added", "group": group_serializer.data}
    ...
    def put(self, request, pk):
        ...
        if not group.members.filter(id=user.id).exists():
            return Response(
                {"detail": "User not a member"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def delete(self, request, pk):
        ...
        if username == request.user.username:
            return Response(
                {"detail": "Cannot remove yourself"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        group_serializer = GroupDetailSerializer(group)
        return Response(
            {"detail": "Member removed", "group": group_serializer.data},
