...
export default function ChangePassword({ hidePassUpdate, emailId }) {
  ...
  const RegisterSchema = Yup.object().shape({
    ...
    newPassword: Yup.string().required('New Password is required').min(8, 'Password should be 8 characters minimum'),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const update_response = await updatePasswordService(values, setShowAlert, setAlertMessage);
      if (update_response) {
        hidePassUpdate();
      }
    },
